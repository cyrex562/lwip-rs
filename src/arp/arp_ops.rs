...
pub fn arp_receive(ctx: &mut LwipContext, table: &mut ArpTable, pkt: &mut PacketBuffer, src_netif: &NetworkInterface) -> Result<(), LwipError> {
    ...
    if result.is_err() {
        ...
        if err.code == LwipErrorCode::NotFound {
            return Err(LwipError::new(LwipErrCode::InvalidArgument, "input packet does not contain an ethernet header"));
        } else {
    ...
    match arp_msg.proto {
        ETHERTYPE_IPV4 => {
            ...
            let from_us: bool = src_netif::has_ip4_addr(&sender_ip4_addr);
            let for_us: bool = src_netif::has_ip4_addr(&tgt_ip4_addr);
            // if !from_us && !for_us {
