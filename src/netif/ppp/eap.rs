        ...
        eapIdentify => {
            ...
            ) != 0)
            {
                ...
                if (id == 0) {
                     ...
                     */
                    if (pcb.settings.eap_timeout_time > 0 && pcb.settings.eap_timeout_time < 30) {
                ...
                *cp2 += 1 = '\0';
                ...
                tpw.pebuf.password.len = t_fromb64(tpw.pwbuf, cp);
                tpw.pebuf.password.data = tpw.pwbuf;
                tpw.pebuf.salt.len = t_fromb64(tpw.saltbuf, cp2);
...
pub fn eap_send_request(pcb: &mut ppp_pcb) {
    ...
    match (pcb.eap.es_server.ea_state) {
        ...
        eapMD5Chall => {
             ...
             */
            pcb.eap.es_challen =
                EAP_MIN_CHALLENGE_LENGTH + magic_pow(EAP_MIN_MAX_POWER_OF_TWO_CHALLENGE_LENGTH);
        ...
        eapSRP1 => {
            ...
            if (ts.n.len != sizeof(wkmodulus) || BCMP(ts.n.data, wkmodulus, sizeof(wkmodulus)) != 0)
            {
            ...
            if (pncrypt_setkey(0)) {
                ...
                *optr = i;
                ...
                while (optr < outp) {
                    ...
                    SHA1pdate(&ctxt, &pcb.eap.es_server.ea_id, 1);
                    SHA1pdate(&ctxt, pcb.eap.es_server.ea_skey, SESSION_KEY_LEN);
                    SHA1pdate(&ctxt, optr - SHA_DIGESTSIZE, SHA_DIGESTSIZE);
                }
            }
        }
...
pub fn eap_rechallenge(arg: &mut Vec<u8>) {
    ...
    if (pcb.eap.es_server.ea_state != eapOpen && pcb.eap.es_server.ea_state != eapSRP4) {
        return;
    }
...
pub fn srp_lwrechallenge(arg: &mut Vec<u8>) {
    ...
    if (pcb.eap.es_server.ea_state != eapOpen || pcb.eap.es_server.ea_type != EAPT_SRP) {
        return;
    }
...
pub fn eap_lowerdown(pcb: &mut ppp_pcb) {
    ...
    if (eap_server_active(pcb)) {
        ...
        if (pcb.eap.es_server.ea_state == eapOpen && pcb.eap.es_lwrechallenge > 0) {
...
pub fn write_pseudonym(esp: &mut eap_state, inp: &mut String, len: usize, id: i32) {
    let val: u8;
            ...
            match (vallen) {
                ...
                EAPSRP_SKEY => {
                    ...
                    if (pcb.eap.es_client.ea_skey != None) {
                        ...
                        if (id != pcb.eap.es_client.ea_id) {
                            ppp_warn(
                                "EAP: ID changed from %d to %d in SRP Subtype 2 rexmit",
                                pcb.eap.es_client.ea_id,
                                id,
                            );
                        }
                    } else {
                ...
                EAPSRP_SVALIDATOR => {
                    ...
                    if (pcb.eap.es_client.ea_state == eapOpen) {
                        if (id != pcb.eap.es_client.ea_id) {
                            ppp_warn(
                                "EAP: ID changed from %d to %d in SRP Subtype 3 rexmit",
                                pcb.eap.es_client.ea_id,
                                id,
                            );
                        }
                        ...
                        if (len < 0 || t_clientverify(tc, inp + sizeof) != 0) {
                            ppp_error("EAP: SRP server verification failed");
                            // goto client_failure;
                        }
...
pub fn eap_response(pcb: &mut ppp_pcb, u_inp: &mut String, id: i32, len: i32) {
            ...
            match (typenum) {
                ...
                EAPSRP_CVALIDATOR => {
                    ...
                    if (len < sizeof + SHA_DIGESTSIZE) {
                        ppp_error("EAP: M1 length %d < %d", len, sizeof + SHA_DIGESTSIZE);
                        eap_figure_next_state(pcb, 1);
                        // break;
                    }
                ...
                EAPSRP_LWRECHALLENGE => {
                    ...
                    if (len != SHA_DIGESTSIZE) {
                        ppp_error("EAP: bad Lightweight rechallenge response");
                        return;
                    }
                    SHA1Init(&ctxt);
                    vallen = id;
    ...
    if (pcb.eap.es_server.ea_state != eapBadAuth && pcb.eap.es_server.ea_state != eapOpen) {
        pcb.eap.es_server.ea_id += 1;
        eap_send_request(pcb);
    }
}
