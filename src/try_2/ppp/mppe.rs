 ...
 *                 providing our own.
 *      2/15/04 - TS: added  and testing for Kernel
 ...
 */
pub fn mppe_rekey(state: &mut ppp_mppe_state, initial_key: i32) {
    let sha1_ctx: lwip_sha1_context;
    let sha1_digest: [u8; SHA1_SIGNATURE_SIZE];
 ...
 */
pub fn mppe_init(pcb: &mut ppp_pcb, state: &mut ppp_mppe_state, options: u8) {
    let debugstr: String = "mppe_comp_init".to_string();
 ...
 */
pub fn mppe_comp_reset(pcb: &mut ppp_pcb, state: &mut ppp_mppe_state) {
 ...
 */
pub fn mppe_compress(
    pcb: &mut ppp_pcb,
    state: &mut ppp_mppe_state,
 ...
 */
pub fn mppe_decomp_reset(pcb: &mut ppp_pcb, state: &mut ppp_mppe_state) {
    return;
}
